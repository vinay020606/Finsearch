...
async def lifespan(app: FastAPI):
    """
    FastAPI Lifespan Context Manager: Initialize Database and Connection Pool.
    """
    logger.info("Initializing Financial RAG Backend API...")
    try:
        init_db_pool()
        init_db()
        logger.info("Database pools and schema initialized successfully.")
...
app = FastAPI(
    ...
    description="FastAPI service with pgvector, RBAC metadata filtering, Hybrid RRF Search, and Cross-Encoder Reranking.",
    version="1.0.0",
    lifespan=lifespan
)
...
def upload_document(payload: DocumentUploadRequest):
    """
    Accepts document data, performs table-aware chunking & embedding, and stores in PostgreSQL with RBAC metadata.
    """
    try:
        ...
        with get_db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                cursor.execute(
                    ...
                    """,
                    (payload.doc_id, payload.ticker_symbol, payload.filename, content_hash, new_version)
                )

                # 5. Delete existing chunks if updating document
                cursor.execute("DELETE FROM financial_chunks WHERE doc_id = %s;", (payload.doc_id,))

                # 6. Insert new chunks & embeddings with RBAC metadata
...
def search_documents(payload: SearchRequest):
    ...
    # 2. Execute 2-Stage Hybrid Search & Reranking
    try:
        with get_db_connection() as conn:
